...
export class MealService {
  ...
  async getMealPacks(_: string[], filters?: IPaginationFilter & { is_available: boolean, country?: string, category: string, orderType:string, tag: string, isSwallow: boolean, continent: string}): Promise<PaginatedDocument<MealPack[]>> {
    // await RoleService.hasPermission(roles, AvailableResource.MEAL, [PermissionScope.READ, PermissionScope.ALL]);
    ...
    if (filters?.isSwallow) {
      Object.assign(queries, { isSwallow: filters.isSwallow });
    }
  ...
  async getSwallowExtras(_: string[], filters?: IPaginationFilter & { name: string, type: string} ): Promise<PaginatedDocument<MealPack[]>> {
  ...
  async getProteinExtras(_: string[], filters?: IPaginationFilter & { name: string, type: string} ): Promise<PaginatedDocument<MealPack[]>> {
  ...
  async getMealExtras(_: string[], filters?: IPaginationFilter & { name: string, type: string}) {
 ...
 async duplicateAndEditMealPacks() {
    try {
      ...
      const existingDuplicatedIds = (await mealPack.find({
        orderType: OrderType.BulkOrder,
      })).map(mealPack => mealPack._id.toString());
  
      const duplicatedMealPacks = mealPacks.map(mealPack => {
        const { _id, ...duplicated } = mealPack.toObject();
        
        if (!existingDuplicatedIds.includes(_id.toString())) {
