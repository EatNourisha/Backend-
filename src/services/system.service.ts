...
import { DefaultAccessControls, PermissionScope, AvailableResource, AvailableRole } from "../valueObjects";
...
export default class SystemService {
  static async ensureResources() {
    ...
    const r = (await RoleService.getResourceByNames(availableResources)).map((resource) => resource?.name);
  ...
  static async ensureRoles() {
    ...
    const r = (await RoleService.getRoleBySlugs(availableRoles)).map((role) => role?.name);
  ...
  static async ensurePermissions(roleId: string, permissions: { name: AvailableResource; scopes: PermissionScope[] }[]) {
    console.log("PERMISSIONS TO ENSURE", String(roleId), permissions);
    const toRun: any = [];

    const resources = await RoleService.getResourceByNames(permissions.map((p) => p.name));

    const getResourceId = (resourceName: string) => resources.find((r) => r.name === resourceName)?._id;
  ...
  static async ensureScopes(roleSlug: string, permissions: { name: AvailableResource; scopes: PermissionScope[] }[]) {
    const toRun: any = [];
  ...
  async ensureSystemServices() {
    const toRun: any = [SystemService.ensureRoles(), SystemService.ensureResources()];
    ...
    const getRoleId = (slug: string) => (roles as Role[])?.find((role) => role.slug === slug)?._id;
    ...
    if (!isEmpty(roles)) {
      unassignedRoles?.forEach(
        (role) =>
          ...
          permissionsToEnsure.push(SystemService.ensurePermissions(getRoleId(role)!, DefaultAccessControls[role].permissions))
      );
      ...
      defaultAccessControlRoles.forEach((role) =>
        permissionsToEnsure.push(SystemService.ensureScopes(role, DefaultAccessControls[role].permissions))
      );

      await Promise.all(permissionsToEnsure);
      defaultAccessControlRoles.forEach((role) => consola.success(capitalize(role)));
      // console.log("ADDED PERMISSION", result);
    }
