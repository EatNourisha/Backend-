...
export class CartService {
  ...
  async getCart(customer_id: string, roles: string[], filters: IPaginationFilter): Promise<CartRo> {
    ...
    const [_cart, items] = await Promise.all([
      ...
      paginate<CartItem[]>("cartItem", { customer: customer_id, quantity: { $gt: 0 } }, filters, { populate: ["item", "swallow", "protein", "swallows", "proteins"] }),
    ]);
  ...
  private static async calcItemPrice(
    ...
    cart_item = await cartItem
      ...
      .populate(["item", "protein", "swallow", "proteins", "swallows"])
      .lean<CartItem>()
      .exec();
