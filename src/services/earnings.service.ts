...
export class EarningsService {
  ...
  async requestWithdrawal(customer_id: string, dto: WithdrawalRequestDto, roles: string[]): Promise<WithdrawalRequest | null> {
    ...
    const last_request = await withdrawalRequest
      .findOne({ customer: customer_id, is_fulfilled: false })
      .select("_id")
      .lean<WithdrawalRequest>()
      .exec();
    ...
    try {
      const txs = await session.withTransaction(async () => {
        ...
        await earnings
          .findByIdAndUpdate(earn._id, { $inc: { balance: -dto?.amount } }, { session })
          .lean()
          .exec();
  ...
  static async getEarnings(customer_id: string): Promise<Earnings> {
    return await earnings
      .findOneAndUpdate({ customer: customer_id }, { customer: customer_id }, getUpdateOptions())
      .lean<Earnings>()
      .exec();
  }
}
