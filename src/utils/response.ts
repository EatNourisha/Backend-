...
export const sendResponse = (res: Response, statusCode: number, result: any, message?: string): void => {
  result = result || { statusCode: 204, status: "success" };
  if (typeof result !== "object") result = result.toObject();
  result = { data: result, version: config.VERSION };
  ...
  res.json(result);
  consola.withTag(result.statusCode || statusCode || 200).success(`Sent Response`);
  console.log("\n");
};
