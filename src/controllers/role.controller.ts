...
export class RoleController {
  ...
  async assignRole(req: Request, res: Response, next: NextFunction) {
    try {
      const result = await service.assignRole(req.params.roleId, req.body.customer_id);
      sendResponse(res, 201, result);
    } catch (error) {
      sendError(error, next);
    }
  ...
  async unassignRole(req: Request, res: Response, next: NextFunction) {
    try {
      const result = await service.unassignRole(req.params.roleId, req.body.customer_id);
      sendResponse(res, 201, result);
    } catch (error) {
      sendError(error, next);
    }
  ...
  async addPermissionScopes(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const result = await service.addPermissionScopes(req.params.roleId, resourceId, scopes);
      sendResponse(res, 201, result);
    } catch (error) {
      sendError(error, next);
    }
  ...
  async revokePermissionScope(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const result = await service.revokePermissionScope(roleId, resourceId, scope as any);
      sendResponse(res, 201, result);
    } catch (error) {
      sendError(error, next);
    }
